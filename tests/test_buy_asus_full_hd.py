...
def test_buy_asus_full_hd(browser):
    m = MainPage(browser)
    m.open()
    ...
    afhd = Asus_full_hd(browser)
    afhd.click_asus_full_hd()
    print("Click add to cart")

    m.click_cart()
    print('Click cart')

    m.assert_values(word=afhd.get_asus_full_hd_name(), result="ASUS Full HD")
    print('Name good')

    price_230 = browser.find_element(By.XPATH, "//tbody[@id='tbodyid']//td[contains(text(), '230')]")
    m.assert_values(price_230, "230")
    print('Price good')
