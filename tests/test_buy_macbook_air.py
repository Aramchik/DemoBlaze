...
def test_buy_macbook_air(browser):
    m = MainPage(browser)
    m.open()
    m.click_laptops()
    print("Click Laptops")
    m.click_macbook_air()
    print("Click Macbook air")

    mbair = Macbook_air(browser)
    mbair.click_macbook_air()
    print("Click add to cart")

    m.click_cart()
    print('Click cart')

    m.assert_values(word=mbair.get_macbook_air_name(), result="MacBook air")
    print('Name good')
